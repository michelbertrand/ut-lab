...
docker run --name realtime-server --network fluig-docker-network \
...
docker run --name index-server --network fluig-docker-network \
...
docker run --name database-server --network fluig-docker-network \
     -p 3306:3306 \
